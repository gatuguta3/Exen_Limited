...
if (empty($installer_id) || empty($date) || empty($status) || empty($serviceId)) {
    http_response_code(400); // Bad Request
...
$sql="UPDATE `services` SET `Status`='$status',`Start_Date`='$date',`Installer_Id`='$installer_id' WHERE Serv_id= '$serviceId'";
